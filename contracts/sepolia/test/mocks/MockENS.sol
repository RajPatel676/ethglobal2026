...
contract MockRegistry is IENSv2Registry {
    struct Entry { address owner; address subregistry; address resolver; uint256 roles; uint64 expires; }
    mapping(bytes32 => Entry) public entries;
    string[] public labels;

    function register(string calldata label, address owner, IENSv2Registry sub, address res, uint256 roles, uint64 expires)
    ...
    function grantRoles(uint256, uint256, address) external pure returns (bool) { return true; }
    function hasRoles(uint256, uint256, address) external pure returns (bool) { return true; }
    function count() external view returns (uint256) { return labels.length; }
}
...
contract MockResolver is IPermissionedResolver {
    ...
    function setWriter(address w) external { writer = w; }
    function setText(bytes32 node, string calldata key, string calldata value) external {
        require(writer == address(0) || msg.sender == writer, "EACUnauthorizedAccountRoles");
        texts[node][key] = value;
    }
    function setAddr(bytes32 node, uint256 coinType, bytes calldata a) external { addrs[node][coinType] = a; }
